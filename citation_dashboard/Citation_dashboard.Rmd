...
server <- function(input, output) {
  observeEvent(input$predict, {
    ...
    output$lm_output <- renderText({
      paste("Linear Model Prediction:", ifelse(round(predict(lm_model, input_pca), 2) > 0, round(predict(lm_model, input_pca), 2) , 0))
    })

    output$rf_output <- renderText({
      paste("Random Forest Prediction:", ifelse(round(predict(rf_model, input_data), 2) > 0, round(predict(rf_model, input_data), 2), 0))
    })
   required_features <- xgb_model$feature_names
   input_data_xgb <- input_data[, required_features]
   print(class(input_data_xgb))
   print(xgb.DMatrix(as.matrix(input_data_xgb)))
    output$xgb_output <- renderText({
      paste("XGBoost Prediction:", ifelse(round(predict(xgb_model, xgb.DMatrix(as.matrix(input_data_xgb))), 2) >0 , round(predict(xgb_model, xgb.DMatrix(as.matrix(input_data_xgb))), 2), 0)) 
    })
    
    output$prediction_plot <- renderPlot({
  lm_prediction <- 0
  lm_prediction <- ifelse(round(predict(lm_model, input_pca), 2) > 0,round(predict(lm_model, input_pca), 2) , 0)
  rf_prediction <- ifelse(round(predict(rf_model, input_data), 2) > 0, round(predict(rf_model, input_data), 2), 0)
  required_features <- xgb_model$feature_names
  input_data_xgb <- input_data[, required_features]
  xgb_prediction <- ifelse(round(predict(xgb_model, xgb.DMatrix(as.matrix(input_data_xgb))), 2) >0, round(predict(xgb_model, xgb.DMatrix(as.matrix(input_data_xgb))), 2), 0 )
